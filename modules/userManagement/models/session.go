...
func SelectSession(sessionToken string) (User, time.Time, error) {
	...
	err := db.QueryRow(`SELECT 
							u.id as user_id, u.type as user_type, u.firstname as user_firstname, u.lastname as user_lastname, u.username as username, u.email as user_email, IFNULL(u.profile_photo, '') as profile_photo,
							expires_at 
						FROM sessions s
							INNER JOIN users u
								ON s.user_id = u.id
						WHERE session_token = ?`, sessionToken).Scan(&user.ID, &user.Type, &user.Firstname, &user.Lastname, &user.Username, &user.Email, &user.ProfilePhoto, &expirationTime)
	if err != nil {
